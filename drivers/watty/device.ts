...
class WattyDevice extends Device {
  ...
  async onInit() {
    ...
    this.#triggers = {
      powerChanged: this.#trigger('watty_power_changed'),
      consumptionChanged: this.#trigger('watty_consumption_changed'),
      costChanged: this.#trigger('watty_cost_changed'),
      currentChanged: {
        L1: this.#trigger('current.L1_changed'),
        L2: this.#trigger('current.L2_changed'),
        L3: this.#trigger('current.L3_changed'),
      },
      dailyConsumptionReport: this.#trigger('watty_daily_consumption_report'),
    };
    this.log(
